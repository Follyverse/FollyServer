...
namespace Multi
{
    ...
    public class PersonClient
    {
        ...
        public bool[] sendableCommon = new bool[6], sendableInput = new bool[8];
        ...
        public byte Sendables()
        {
            ...
            for (int i = 0; i < 6; i++)
            {
                if (sendableCommon[i] = commonChanged[i])
                {
                    r++;
            //        if (i == 3) Debug.Log("rot changed " + id);
                }
                commonChanged[i] = false;
            }
            for (int i = 0; i < 8; i++)
            {
                if (sendableInput[i] = inputChanged[i])
                {
                    r++;
                }
